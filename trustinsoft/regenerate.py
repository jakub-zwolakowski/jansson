...
c_tests = sorted(glob.glob(path.join("test", "suites", "api", "test_*.c")))
...
def make_c_test(test_path, machdep):
    c_file = path.basename(test_path)
    test_name = c_file.replace("test_", "").replace(".c", "")
    suite_name = path.basename(path.dirname(test_path))
    tis_test = {
        "name":
            "SUITE %s, TEST %s, ARCH %s"
            % (suite_name, test_name, machdep["pretty_name"]),
...
def make_tis_config():
    tis_c_tests = product(c_tests, machdeps)
    tis_process_tests = product(process_tests, machdeps)
    return (
        list(map(
            lambda t: make_c_test(t[0], t[1]),
            tis_c_tests
        ))
        +
        list(map(
